...
function Cab(){
  ...
  $cn->close($query);
  $cn = new PostgreSQL();
	$query = $cn->consulta("SELECT p.rucproveedor,p.razonsocial,p.direccion,p.distritoid,p.provinciaid,p.departamentoid,p.paisid,c.contacto FROM admin.proveedor p INNER JOIN logistica.compras c ON c.rucproveedor=p.rucproveedor WHERE p.rucproveedor LIKE '".$this->ruc_."' AND  c.nrocompra LIKE '".$this->nro_."'");
	if ($cn->num_rows($query)>0) {
		while ($result = $cn->ExecuteNomQuery($query)) {
      $this->SetXY(10,38);
      ...
      $this->SetXY(10,50);
			$this->cell(10,4,'Distrito:              '.$result['distrito'],'L',1,'L',false);
			$this->SetXY(55,50);
      $this->cell(10,4,'Provincia: '.$result['provincia'],0,1,'L',false);
			$this->SetXY(10,54);
      $this->cell(90,4,'Departamento:   '.$result['departamento'],'L',1,'L',false);
			$this->SetXY(55,54);
      $this->cell(45,4,'Pais:      '.$result['pais'],'R',1,'L',false);
      $this->SetXY(10,58);
