	...
	<div id="cabos">
		<?php
			...
			$query = $cn->consulta("SELECT s.nrosuministro,a.descri,e.empnom,s.fecha::date,s.fecreq,d.esnom
									...
									ON s.esid=d.esid
									WHERE s.esid LIKE '40' AND s.nrosuministro LIKE '".$_GET['nros']."'");
	...
	<table id="tbldet">
		...
		<tbody>
			<?php
				$cn = new PostgreSQL();
				$query = $cn->consulta("
					SELECT DISTINCT d.materialesid,m.matnom,m.matmed,m.matund,d.cantidad 
					FROM almacen.inventario i INNER JOIN almacen.detsuministro d
