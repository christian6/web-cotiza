...
if (isset($_GET['send'])) {
	$cn = new PostgreSQL();
	$query = $cn->consulta("
		UPDATE admin.clientes SET nombre = '".$_GET['txtrz']."', abre = '".$_GET['txtabre']."',direccion = '".$_GET['txtdir']."',
		paisid = '".$_GET['cbopais']."', departamentoid = '".$_GET['cbodepartamento']."', provinciaid = '".$_GET['cboprovincia']."',
		distritoid = '".$_GET['cbodistrito']."', telefono = '".$_GET['txttel']."', contacto = '".$_GET['txtcont']."'
		WHERE ruccliente = '".$_GET['ruc']."'
		");
...
<html>
<head>
	<title>Editar Cliente <?php echo $_REQUEST['ruc'] ?></title>
...
<div class="row-fluid show-grid">
	<div class="well">
		<h5>Editar Cliente Ruc: <?php echo $_REQUEST['ruc'];?></h5>
	</div>
	<div class="row well">
<?php
	...
	$cn = new PostgreSQL();
	$query = $cn->consulta("
		SELECT c.ruccliente,c.nombre,c.abre,c.direccion,c.paisid,a.paisnom,c.departamentoid,d.deparnom,c.provinciaid,r.provnom,c.distritoid,i.distnom,c.telefono,c.contacto,e.esnom
						...
						ON c.esid=e.esid
						WHERE a.paisid LIKE d.paisid AND d.departamentoid LIKE r.departamentoid AND r.provinciaid LIKE i.provinciaid AND c.ruccliente LIKE '".$_REQUEST['ruc']."' AND c.esid LIKE '41'
...
?>
		<form name="frmedit" class="well form-block" method="GET" action="">
			<div class="row">
				...
				<input type="text" name="txtrz" class="span6" value="<?php echo $res['nombre']?>" title="Razon Social" placeholder="Ingrese Razon Social" REQUIRED>
				<labelfor="lblabre">Iniciales:</label>
				<input type="text" name="txtabre" class="input-small" value="<?php echo $res['abre']?>" title="Iniciales" placeholder="Iniciales" REQUIRED>
				...
				<label for="lblcont">Contacto:</label>
				<input type="text" name="txtcont" class="span6" value="<?php echo $res['contacto'];?>" title="Ingrese Contacto" placeholder="Ingrese Contacto" REQUIRED />
				<p>
